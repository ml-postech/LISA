...
def validate(val_loader, model):
    ...
    return (val_res.item(), val_res_snr.item()), \
        (torch.cat(val_samples, dim=0), setup['sr']), val_res_lsd.item()
...
def main(config_, save_path, rank=0, num_gpus=1):
    ...
    for epoch in range(epoch_start, epoch_max + 1):
        ...
        if (epoch_val is not None) and (epoch % epoch_val == 0):
            ...
            val_res, val_sample, val_lsd = validate(val_loader, model_)


            log_info.append('val: snr={:.4f}'.format(val_res[1]))
            log_info.append('lsd = {:.4f}'.format(val_lsd))
            # if torch.cuda.device_count() > 1:
            #     val_res_ = torch.Tensor([val_res]).cuda()
            #     dist.reduce(val_res_, 0)
            #     val_res = val_res_.cpu().item() / float(dist.get_world_size())
            if writer is not None:
                writer.add_scalar('sdr', val_res[0], epoch)
                writer.add_scalar('snr', val_res[1], epoch)
                writer.add_scalar('lsd', val_lsd, epoch)
                writer.add_audio(tag="sample/val", snd_tensor=val_sample[0].view(-1), global_step=epoch, sample_rate=val_sample[1])
            if val_res[1] > max_val_v:
                max_val_v = val_res[1]
